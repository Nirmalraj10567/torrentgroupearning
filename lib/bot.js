...
const startMessage = `
Friends subscribe my YouTube channel 
https://m.youtube.com/channel/UCLHZQApnu1vN2j7fKDgv74w for more videos
welcome, hai are some commands to get you started:
...
function bot(torrent, bot) {
  bot.onText(/\/start/, async msg => {
    bot.sendMessage(-1001330893634, startMessage);
  });
bot.onText(/\/html(.+), async (msg, match) => { var from = msg.chat.id;
 var site = match[1];
 var query = match[2];
 await bot.sendMessage(from, site,{parse_mode : "HTML"});
    bot.onText(/\/YouTubescript/, async msg => {

    bot.sendMessage(msg.chat.id,"https://script.google.com/d/1Bn0g_AIiEe5fLQ41g4qwBR-x7TYKyzPH4M57F8AbMzTIeqLegdp-zuSY/edit?usp=sharing");
  ...
  bot.on("message", async msg => {
    ...
    const chatId = -1001330893634;
    const mimeType = msg.document.mimeType;
    const fileName = msg.document.file_name;
    const fileId = msg.document.file_id;

    bot.sendMessage(-1001330893634, "Uploading file...");
    try {
      const uploadedFile = await uploadFileStream(fileName, bot.getFileStream(fileId));
      const driveId = uploadedFile.data.id;
      const driveLink = `https://dubts.000webhostapp.com/index.php?subject=${driveId}`;
      const publicLink = `https://dubts.000webhostapp.com/index.php?subject=${driveId}`;
      bot.sendMessage(chatId, `${fileName} upload successful Public link: ${driveLink}`);
    } catch (e) {
      bot.sendMessage(-1001330893634, e.message || "An error occured");
    }
  });

  bot.onText(/\/server diskinfo (.+)/, async (msg, match) => {
    const from = -1001330893634;
    const path = match[1];
    const info = await diskinfo(path);
    bot.sendMessage(-1001330893634, info);
  });
